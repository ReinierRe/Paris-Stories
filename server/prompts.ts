...
export const themeAngleDefinitions: Record<string, { id: string; names: Record<string, string>; descriptions: Record<string, string> }[]> = {
  "golden-age": [
...
export const topicToThemeMap: Record<string, string> = {
...
export function getSiblingAngles(topicId: string | undefined, currentPerspective: string, language: string): SiblingAngle[] {
...
export function resolvePerspectiveText(perspective: string, langKey: string, city?: City): string {
  ...
  return perspective
    ? (curatedPerspectiveMap[perspective]?.[langKey] || defaultStyle[langKey])
