...
## Recent Changes
- 2026-03-29: Moved topic-theme mapping to database. Added 3 JSONB columns to `cities` table: `topicThemeMap` (topic→theme mapping), `themeAngles` (angle definitions per theme), `perspectivePrompts` (optional city-specific perspective text overrides). `getSiblingAngles()` now reads from `city.topicThemeMap` and `city.themeAngles` instead of hardcoded maps. `resolvePerspectiveText()` fallback chain: city.perspectivePrompts → curatedPerspectiveMap → defaultStyle. Removed hardcoded `topicToThemeMap`, `themeAngleDefinitions`, and dead `iconic-figures` alias from `server/prompts.ts`. Migration script: `scripts/topic-theme-migration.sql`. New cities only need DB records — no server code changes.
