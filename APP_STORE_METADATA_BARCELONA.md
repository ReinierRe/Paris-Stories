# App Store Connect Metadata — Barcelona Stories

Kopieer onderstaande teksten naar de juiste velden in App Store Connect.
Alle secties zijn klaar om in te vullen.

---

## 1. App Information (General)

**App Name**: Barcelona Stories
**Bundle ID**: app.replit.barcelonastories
**Primary Language**: Spanish (Spain)
**Category**: Education
**Secondary Category**: Travel

---

## 2. Localized Metadata (per language)

Voeg in App Store Connect elke taal toe via "Localizations" en kopieer de bijbehorende teksten.

---

### Spanish (Español) — Primary

**Subtitle** (max 30 tekens):
```
Podcasts de Barcelona a medida
```

**Description** (max 4000 tekens):
```
Descubre la verdadera Barcelona a través de podcasts generados por inteligencia artificial. Barcelona Stories crea guías de audio personalizadas sobre la historia, la cultura, la gastronomía, el arte y los rincones ocultos de la capital catalana — creadas especialmente para ti.

CÓMO FUNCIONA
Explora nuestra biblioteca de temas barceloneses — desde la Barcino romana hasta las obras maestras de Gaudí, desde los secretos del Barrio Gótico hasta los castellers de las plazas. Elige un tema, selecciona tu ángulo narrativo favorito y Barcelona Stories genera un episodio único y basado en hechos, adaptado a tus intereses.

BIBLIOTECA
Explora temas en siete categorías ricas:
- Historia — Desde Barcino y la Corona de Aragón hasta el Sitio de 1714 y la Renaixença
- Gaudí y Modernismo — Sagrada Família, Park Güell, Casa Batlló, La Pedrera y el genio de Antoni Gaudí
- Museos — Museu Picasso, MNAC, Fundació Miró, MACBA y más
- Edificios y Monumentos — La Catedral, Santa Maria del Mar, Palau de la Música, Camp Nou
- Historia Moderna — El FC Barcelona y Cruyff (més que un club), la transición, las superilles y los castellers
- Barrios — Gòtic, El Born, El Raval, Gràcia, Barceloneta, Poble Sec y Poblenou
- Gastronomía — Tapas, pa amb tomàquet, vermut, mariscos de la Barceloneta, cava y crema catalana

PODCASTS PERSONALIZADOS
¿Quieres aprender sobre algo específico? Crea tu propio podcast sobre cualquier tema relacionado con Barcelona. Ingresa tu tema, elige un ángulo narrativo y recibe un episodio único en minutos.

MÚLTIPLES IDIOMAS Y VOCES
Escucha en inglés, neerlandés, francés, alemán o español. Elige entre una voz masculina o femenina. Configura tus preferencias una vez en tu perfil — cada podcast se adapta automáticamente.

CARACTERÍSTICAS PRINCIPALES
- Guiones generados por IA con contenido factual y bien investigado
- Audio de sonido natural con Google Cloud Text-to-Speech
- Reproductor interactivo con barra de progreso deslizable
- Guarda y vuelve a escuchar tus episodios favoritos
- Interfaz limpia e intuitiva diseñada para descubrir
- Moderación de contenido que garantiza temas apropiados

Barcelona Stories es perfecto para viajeros que preparan un viaje a Barcelona, entusiastas de la historia, amantes del arte y la arquitectura, estudiantes de idiomas o cualquier persona curiosa por una de las ciudades más fascinantes del Mediterráneo.

Todo el contenido se genera mediante inteligencia artificial y se convierte en voz de sonido natural. Aunque nos esforzamos por la precisión, el contenido generado por IA puede contener ocasionalmente inexactitudes.
```

**Promotional Text** (max 170 tekens):
```
Descubre Barcelona como nunca. Podcasts generados por IA sobre Gaudí, el Barrio Gótico, la cultura mediterránea y rincones ocultos — en tu idioma, con tu voz.
```

**Keywords** (max 100 tekens):
```
barcelona,podcast,viaje,guía audio,gaudí,cataluña,IA,cultura,arte,modernismo,sagrada família
```

**What's New**:
```
¡Bienvenido a Barcelona Stories! Descubre la verdadera Barcelona a través de podcasts generados por IA sobre Gaudí, el Barrio Gótico, la cultura catalana y rincones ocultos.
```

---

### English (U.K.)

**Subtitle**:
```
Personalized Barcelona Podcasts
```

**Description**:
```
Discover the real Barcelona through AI-powered podcast stories. Barcelona Stories creates personalized audio guides about the history, culture, food, art, and hidden gems of the Catalan capital — generated just for you.

HOW IT WORKS
Browse our curated library of Barcelona topics — from Roman Barcino to Gaudí's masterpieces, from the secrets of the Gothic Quarter to the human-tower castellers of the plaças. Pick a topic, choose your preferred storytelling angle, and Barcelona Stories generates a unique, factual podcast episode tailored to your interests.

CURATED LIBRARY
Explore topics across seven rich categories:
- History — From Barcino and the Crown of Aragon to the Siege of 1714 and the Renaixença
- Gaudí & Modernism — Sagrada Família, Park Güell, Casa Batlló, La Pedrera, and the genius of Antoni Gaudí
- Museums — Museu Picasso, MNAC, Fundació Miró, MACBA, and more
- Buildings & Monuments — The Cathedral, Santa Maria del Mar, Palau de la Música, Camp Nou
- Modern History — FC Barcelona & Cruyff (més que un club), the transition, the superblocks, and castellers
- Neighborhoods — Gòtic, El Born, El Raval, Gràcia, Barceloneta, Poble Sec, and Poblenou
- Culinary — Tapas, pa amb tomàquet, vermut, Barceloneta seafood, cava, and crema catalana

CUSTOM PODCASTS
Want to learn about something specific? Create your own podcast about any Barcelona-related topic. Enter your subject, pick a storytelling angle, and get a unique episode in minutes.

MULTIPLE LANGUAGES & VOICES
Listen in English, Dutch, French, German, or Spanish. Choose between a male or female voice. Set your preferences once in your profile — every podcast adapts automatically.

KEY FEATURES
- AI-generated scripts with factual, well-researched content
- Natural-sounding audio powered by Google Cloud Text-to-Speech
- Interactive player with seekable progress bar
- Save and replay your favorite episodes
- Clean, intuitive interface designed for discovery
- Content moderation ensures appropriate topics

Barcelona Stories is perfect for travelers preparing a trip to Barcelona, history enthusiasts, art and architecture lovers, language learners, or anyone curious about one of the most fascinating Mediterranean cities.

All content is generated using artificial intelligence and converted to natural-sounding speech. While we strive for accuracy, AI-generated content may occasionally contain inaccuracies.
```

**Promotional Text**:
```
Explore Barcelona like never before. AI-powered podcast stories about Gaudí, the Gothic Quarter, Mediterranean culture, and hidden gems — in your language, your voice.
```

**Keywords**:
```
barcelona,podcast,travel,audio guide,gaudi,catalonia,AI,stories,culture,art,modernism
```

**What's New**:
```
Welcome to Barcelona Stories! Discover the real Barcelona through AI-powered podcast stories about Gaudí, the Gothic Quarter, Catalan culture, and hidden gems.
```

---

### Dutch (Nederlands)

**Subtitle**:
```
Jouw Barcelona Podcastgids
```

**Description**:
```
Ontdek het echte Barcelona met AI-podcastverhalen. Barcelona Stories maakt persoonlijke audiogidsen over de geschiedenis, cultuur, het eten, de kunst en de verborgen parels van de Catalaanse hoofdstad — speciaal voor jou gemaakt.

HOE HET WERKT
Blader door onze bibliotheek met Barcelona-onderwerpen — van Romeins Barcino tot de meesterwerken van Gaudí, van de geheimen van de Gotische Wijk tot de castellers op de plaças. Kies een onderwerp, selecteer je favoriete invalshoek en Barcelona Stories genereert een unieke, feitelijke podcastaflevering op maat.

BIBLIOTHEEK
Verken onderwerpen in zeven rijke categorieën:
- Geschiedenis — Van Barcino en de Kroon van Aragon tot het Beleg van 1714 en de Renaixença
- Gaudí & Modernisme — Sagrada Família, Park Güell, Casa Batlló, La Pedrera en het genie Antoni Gaudí
- Musea — Museu Picasso, MNAC, Fundació Miró, MACBA en meer
- Gebouwen & Monumenten — De Kathedraal, Santa Maria del Mar, Palau de la Música, Camp Nou
- Moderne Geschiedenis — FC Barcelona & Cruyff (més que un club), de transitie, superilles en castellers
- Buurten — Gòtic, El Born, El Raval, Gràcia, Barceloneta, Poble Sec en Poblenou
- Culinair — Tapas, pa amb tomàquet, vermut, zeevruchten uit de Barceloneta, cava en crema catalana

EIGEN PODCASTS
Wil je meer weten over een specifiek onderwerp? Maak je eigen podcast over elk Barcelona-gerelateerd thema. Voer je onderwerp in, kies een invalshoek en ontvang een unieke aflevering binnen enkele minuten.

MEERDERE TALEN & STEMMEN
Luister in het Engels, Nederlands, Frans, Duits of Spaans. Kies tussen een mannelijke of vrouwelijke stem. Stel je voorkeuren eenmalig in via je profiel — elke podcast past zich automatisch aan.

BELANGRIJKSTE FUNCTIES
- AI-gegenereerde scripts met feitelijke, goed onderzochte inhoud
- Natuurlijk klinkende audio via Google Cloud Text-to-Speech
- Interactieve speler met versleepbare voortgangsbalk
- Bewaar en herbeluister je favoriete afleveringen
- Overzichtelijke, intuïtieve interface ontworpen voor ontdekking
- Inhoudsmoderatie zorgt voor gepaste onderwerpen

Barcelona Stories is perfect voor reizigers die een trip naar Barcelona voorbereiden, geschiedenisliefhebbers, kunst- en architectuurliefhebbers, taalleerders of iedereen die nieuwsgierig is naar een van de meest fascinerende mediterrane steden.

Alle inhoud wordt gegenereerd met kunstmatige intelligentie en omgezet naar natuurlijk klinkende spraak. Hoewel we streven naar nauwkeurigheid, kan AI-gegenereerde inhoud af en toe onjuistheden bevatten.
```

**Promotional Text**:
```
Ontdek Barcelona zoals nooit tevoren. AI-podcastverhalen over Gaudí, de Gotische Wijk, de Catalaanse cultuur en verborgen parels — in jouw taal, met jouw stem.
```

**Keywords**:
```
barcelona,podcast,reisgids,audiogids,gaudí,catalonië,verhalen,cultuur,kunst,modernisme
```

**What's New**:
```
Welkom bij Barcelona Stories! Ontdek het echte Barcelona met AI-podcastverhalen over Gaudí, de Gotische Wijk, de Catalaanse cultuur en verborgen parels.
```

---

### French (Français)

**Subtitle**:
```
Podcasts Barcelone sur mesure
```

**Description**:
```
Découvrez la vraie Barcelone à travers des podcasts générés par intelligence artificielle. Barcelona Stories crée des guides audio personnalisés sur l'histoire, la culture, la gastronomie, l'art et les trésors cachés de la capitale catalane — créés spécialement pour vous.

COMMENT ÇA MARCHE
Parcourez notre bibliothèque de sujets barcelonais — de la Barcino romaine aux chefs-d'œuvre de Gaudí, des secrets du Quartier Gothique aux castellers des plaças. Choisissez un sujet, sélectionnez votre angle narratif préféré, et Barcelona Stories génère un épisode de podcast unique et factuel, adapté à vos centres d'intérêt.

BIBLIOTHÈQUE
Explorez des sujets dans sept catégories riches :
- Histoire — De Barcino et de la Couronne d'Aragon au Siège de 1714 et à la Renaixença
- Gaudí & Modernisme — Sagrada Família, Park Güell, Casa Batlló, La Pedrera et le génie d'Antoni Gaudí
- Musées — Museu Picasso, MNAC, Fondation Miró, MACBA et plus
- Bâtiments & Monuments — La Cathédrale, Santa Maria del Mar, Palau de la Música, Camp Nou
- Histoire moderne — Le FC Barcelone et Cruyff (més que un club), la transition, les superblocs et les castellers
- Quartiers — Gòtic, El Born, El Raval, Gràcia, Barceloneta, Poble Sec et Poblenou
- Gastronomie — Tapas, pa amb tomàquet, vermut, fruits de mer de la Barceloneta, cava et crema catalana

PODCASTS PERSONNALISÉS
Envie d'en savoir plus sur un sujet précis ? Créez votre propre podcast sur n'importe quel thème lié à Barcelone. Entrez votre sujet, choisissez un angle narratif et recevez un épisode unique en quelques minutes.

PLUSIEURS LANGUES & VOIX
Écoutez en anglais, néerlandais, français, allemand ou espagnol. Choisissez entre une voix masculine ou féminine. Réglez vos préférences une fois dans votre profil — chaque podcast s'adapte automatiquement.

FONCTIONNALITÉS CLÉS
- Scripts générés par IA avec un contenu factuel et bien documenté
- Audio au son naturel grâce à Google Cloud Text-to-Speech
- Lecteur interactif avec barre de progression
- Sauvegardez et réécoutez vos épisodes favoris
- Interface claire et intuitive conçue pour l'exploration
- Modération du contenu pour garantir des sujets appropriés

Barcelona Stories est parfait pour les voyageurs préparant un séjour à Barcelone, les passionnés d'histoire, les amateurs d'art et d'architecture, les apprenants en langues ou toute personne curieuse de découvrir l'une des villes méditerranéennes les plus fascinantes.

Tout le contenu est généré par intelligence artificielle et converti en parole au son naturel. Bien que nous visions la précision, le contenu généré par IA peut occasionnellement contenir des inexactitudes.
```

**Promotional Text**:
```
Explorez Barcelone comme jamais. Des podcasts générés par IA sur Gaudí, le Quartier Gothique, la culture méditerranéenne et les trésors cachés — dans votre langue.
```

**Keywords**:
```
barcelone,podcast,voyage,guide audio,gaudí,catalogne,IA,culture,art,modernisme,visite
```

**What's New**:
```
Bienvenue dans Barcelona Stories ! Découvrez la vraie Barcelone à travers des podcasts générés par IA sur Gaudí, le Quartier Gothique, la culture catalane et les trésors cachés.
```

---

### German (Deutsch)

**Subtitle**:
```
Dein Barcelona-Podcast
```

**Description**:
```
Entdecke das echte Barcelona mit KI-generierten Podcast-Geschichten. Barcelona Stories erstellt personalisierte Audio-Guides über die Geschichte, Kultur, Küche, Kunst und verborgenen Schätze der katalanischen Hauptstadt — speziell für dich gemacht.

SO FUNKTIONIERT ES
Durchstöbere unsere Bibliothek mit Barcelona-Themen — vom römischen Barcino bis zu den Meisterwerken Gaudís, von den Geheimnissen des Gotischen Viertels bis zu den Castellers auf den Plätzen. Wähle ein Thema, suche dir deinen bevorzugten Erzählwinkel aus und Barcelona Stories generiert eine einzigartige, faktenbasierte Podcast-Folge nach deinem Geschmack.

BIBLIOTHEK
Entdecke Themen in sieben reichhaltigen Kategorien:
- Geschichte — Von Barcino und der Krone Aragon bis zur Belagerung 1714 und der Renaixença
- Gaudí & Modernismus — Sagrada Família, Park Güell, Casa Batlló, La Pedrera und das Genie Antoni Gaudí
- Museen — Museu Picasso, MNAC, Fundació Miró, MACBA und mehr
- Gebäude & Denkmäler — Die Kathedrale, Santa Maria del Mar, Palau de la Música, Camp Nou
- Moderne Geschichte — Der FC Barcelona und Cruyff (més que un club), der Übergang, die Superblocks und die Castellers
- Viertel — Gòtic, El Born, El Raval, Gràcia, Barceloneta, Poble Sec und Poblenou
- Kulinarik — Tapas, pa amb tomàquet, Vermut, Meeresfrüchte aus der Barceloneta, Cava und Crema Catalana

EIGENE PODCASTS
Möchtest du mehr über ein bestimmtes Thema erfahren? Erstelle deinen eigenen Podcast zu jedem Barcelona-Thema. Gib dein Thema ein, wähle einen Erzählwinkel und erhalte eine einzigartige Folge in wenigen Minuten.

MEHRERE SPRACHEN & STIMMEN
Höre auf Englisch, Niederländisch, Französisch, Deutsch oder Spanisch. Wähle zwischen einer männlichen oder weiblichen Stimme. Lege deine Vorlieben einmal in deinem Profil fest — jeder Podcast passt sich automatisch an.

HAUPTFUNKTIONEN
- KI-generierte Skripte mit faktenbasiertem, gut recherchiertem Inhalt
- Natürlich klingendes Audio mit Google Cloud Text-to-Speech
- Interaktiver Player mit verschiebbarem Fortschrittsbalken
- Speichere und höre deine Lieblingsfolgen erneut
- Übersichtliche, intuitive Oberfläche zum Entdecken
- Inhaltsmoderation sorgt für angemessene Themen

Barcelona Stories ist perfekt für Reisende, die eine Barcelona-Reise planen, Geschichtsbegeisterte, Kunst- und Architekturliebhaber, Sprachlerner oder alle, die neugierig auf eine der faszinierendsten Mittelmeerstädte sind.

Alle Inhalte werden mit künstlicher Intelligenz generiert und in natürlich klingende Sprache umgewandelt. Obwohl wir Genauigkeit anstreben, können KI-generierte Inhalte gelegentlich Ungenauigkeiten enthalten.
```

**Promotional Text**:
```
Entdecke Barcelona wie nie zuvor. KI-generierte Podcast-Geschichten über Gaudí, das Gotische Viertel, mediterrane Kultur und verborgene Schätze — in deiner Sprache.
```

**Keywords**:
```
barcelona,podcast,reise,reiseführer,gaudí,katalonien,KI,kultur,kunst,modernismus,museen
```

**What's New**:
```
Willkommen bei Barcelona Stories! Entdecke das echte Barcelona mit KI-generierten Podcast-Geschichten über Gaudí, das Gotische Viertel, katalanische Kultur und verborgene Schätze.
```

---

## 3. Non-Localized Sections

### Support URL
```
https://paris-stories.replit.app/privacy-policy?city=barcelona
```

### Privacy Policy URL
```
https://paris-stories.replit.app/privacy-policy?city=barcelona
```

---

## 4. Age Rating Questionnaire

| Vraag | Antwoord |
|-------|----------|
| Cartoon or Fantasy Violence | None |
| Realistic Violence | None |
| Prolonged Graphic or Sadistic Realistic Violence | None |
| Profanity or Crude Humor | None |
| Mature/Suggestive Themes | None |
| Horror/Fear Themes | None |
| Medical/Treatment Information | None |
| Alcohol, Tobacco, or Drug Use or References | Infrequent/Mild |
| Simulated Gambling | None |
| Sexual Content or Nudity | None |
| Contests | None |
| Unrestricted Web Access | No |
| Gambling with Real Currency | No |

**Resultaat**: 4+ (Geschikt voor alle leeftijden)

**Toelichting**: "Infrequent/Mild" bij alcohol omdat podcasts over Catalaanse eetcultuur cava, vermut en wijn kunnen bespreken.

---

## 5. App Privacy (Privacy Labels)

Identiek aan Paris Stories en Amsterdam Stories. Configureer in App Store Connect > App Privacy:

#### A. Contact Info — Email Address
- Collected: Yes / Linked to User: Yes / Tracking: No / Purpose: App Functionality

#### B. User Content
- Collected: Yes (custom podcast topics) / Linked to User: Yes / Tracking: No / Purpose: App Functionality

#### C. Identifiers — User ID
- Collected: Yes (Firebase UID) / Linked to User: Yes / Tracking: No / Purpose: App Functionality

#### D. Usage Data — Product Interaction
- Collected: Yes (podcast generation history) / Linked to User: Yes / Tracking: No / Purpose: App Functionality

### Third-Party Data Processing
- **Anthropic Claude** — Ontvangt podcast-onderwerpen voor scriptgeneratie
- **Google Cloud Text-to-Speech** — Ontvangt gegenereerde scripts voor audio-omzetting
- **Firebase Authentication** — Beheert login/registratie

### Data Used to Track You
**No**

---

## 6. App Review Information

### Sign-in Required
**Yes**

### Demo Account
- **Email**: reviewer@barcelonastories.app
- **Password**: TestReview2026!

### App Review Notes
```
Barcelona Stories generates AI-powered podcast stories about Barcelona. The app uses Anthropic Claude (AI) to create factual scripts and Google Cloud Text-to-Speech to convert them to audio.

TO TEST THE APP:
1. Log in with the provided demo account credentials
2. Go to the "Library" tab to browse curated Barcelona topics
3. Select any topic (e.g., "La Sagrada Família" under Gaudí & Modernism)
4. Choose a storytelling angle (e.g., "Historical") and tap "Generate Podcast"
5. Wait approximately 30-60 seconds for AI generation
6. The podcast will appear in "My Podcasts" — tap to play

TO TEST CUSTOM PODCASTS:
1. Go to "My Podcasts" tab and tap "Create Custom Podcast"
2. Enter a Barcelona-related topic (e.g., "The history of vermut in Barcelona")
3. Choose a storytelling angle and generate
4. Custom topics are moderated by AI — inappropriate subjects are rejected

ADDITIONAL FEATURES:
- Profile tab: Change language (EN/NL/FR/DE/ES) and voice (male/female) preferences
- Player: Interactive seekable progress bar, read-along script
- Delete Account: Available in Profile > Delete Account (Apple requirement)
- Privacy Policy: Accessible from Profile and Login screens

Note: Podcast generation requires an active internet connection and takes 30-60 seconds per episode. All content is AI-generated and moderated.
```

---

## 7. Screenshots

### Aanbevolen screenshots (5-8 stuks):
1. **Library** — Hoofdscherm met categorieën (History, Gaudí, Museums, etc.)
2. **Topic keuze** — Gaudí-categorie open met Sagrada Família etc.
3. **Podcast speler** — Player met Sagrada Família verhaal
4. **Custom podcast** — Scherm voor eigen onderwerp
5. **My Podcasts** — Lijst met opgeslagen podcasts
6. **Profile** — Profielscherm met taal/stem
7. **Login** — Welkomstscherm met onboarding slides

### Specs:
- **iPhone 6.7"**: 1290 x 2796 px
- **iPhone 6.5"**: 1284 x 2778 px
- **iPad 12.9"** (optioneel): 2048 x 2732 px

---

## 8. Export Compliance

**Does your app use encryption?** → **Yes**
**Does your app qualify for any of the exemptions?** → **Yes**
Selecteer: "Your app uses standard encryption exempt from EAR regulations."

---

## 9. Copyright

```
© 2026 GreenHome
```

---

## 10. Build & Submission Checklist

- [ ] App aangemaakt in App Store Connect (noteer ascAppId voor eas.json)
- [ ] Nieuw Expo.dev project aangemaakt (vul `EXPO_PUBLIC_EAS_PROJECT_ID` in eas.json voor `:barcelona` profielen)
- [ ] App icon (1024x1024 px, geen transparantie, geen afgeronde hoeken)
- [ ] Screenshots voor alle vereiste schermformaten (6.7" en 6.5")
- [ ] Demo account aangemaakt in Firebase Console (reviewer@barcelonastories.app)
- [ ] Privacy Policy URL live en bereikbaar (?city=barcelona)
- [ ] Support URL ingevuld
- [ ] Copyright ingevuld (© 2026 GreenHome)
- [ ] Leeftijdsclassificatie vragenlijst ingevuld
- [ ] Privacy labels geconfigureerd
- [ ] App Review Notes ingevuld met demo credentials
- [ ] Sign-in Required = Yes
- [ ] Export Compliance beantwoord
- [ ] EAS Build: `eas build --platform ios --profile production:barcelona`
- [ ] Build geüpload naar App Store Connect
- [ ] ascAppId ingevuld in eas.json (`submit.production:barcelona`)
- [ ] Localisaties toegevoegd: ES, EN, NL, FR, DE
